...
public:
  ...
  nsresult Init() MOZ_OVERRIDE {
    GLuint tex = mozilla::gl::TexturePoolOGL::AcquireTexture();
    if (!tex) {
  ...
  nsresult PostOutput(BufferInfo* aInfo) {
    VideoInfo videoInfo;
...
public:
  ...
  nsresult Output(BufferInfo* aInfo, void* aBuffer) {
    ...
    int16_t* data = reinterpret_cast<int16_t*>(aBuffer);
    ...
    mCallback->Output(new AudioData(aInfo->getOffset(), aInfo->getPresentationTimeUs(), 0,
                                    numFrames,
                                    audio,
                                    numChannels,
